...
describe('release packaging configuration', () => {
  ...
  it('uploads mac zip artifacts without a stale Windows zip glob', () => {
    ...
    expect(workflow).toContain('out/threetwoa-*-mac-*.zip');
    expect(workflow).not.toContain('out/threetwoa-*-win32-*.zip');
  });
  ...
  itWithBash('fails release asset preparation when a mac zip is missing', () => {
    ...
    try {
      ...
      rmSync(resolve(artifactsDir, 'macos-build-arm64', 'threetwoa-1.0.0-mac-arm64.zip'), { force: true });
