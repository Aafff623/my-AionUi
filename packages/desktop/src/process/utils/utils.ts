...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'threetwoa');
};
 ...
 * Release builds use ~/.threetwoa; dev builds use ~/.threetwoa-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.threetwoa，Dev 模式使用 ~/.threetwoa-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'threetwoa');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.threetwoa'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.threetwoa-config; dev builds use ~/.threetwoa-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.threetwoa-config，Dev 模式使用 ~/.threetwoa-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.threetwoa-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[threetwoa] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[threetwoa] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[threetwoa] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[threetwoa] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
