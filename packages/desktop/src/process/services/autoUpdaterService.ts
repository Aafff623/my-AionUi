...
class AutoUpdaterService extends EventEmitter {
  ...
  constructor() {
    ...
    const updateFeedOptions = buildUpdateFeedOptions();
    ...
    autoUpdater.setFeedURL(updateFeedOptions);
    log.info('Update feed set to GitHub provider (fork releases)');
    log.debug('[auto-update] GitHub feed configured', {
      provider: updateFeedOptions.provider,
      owner: updateFeedOptions.owner,
      repo: updateFeedOptions.repo,
  ...
  private ensureDevUpdateConfig(): void {
    try {
      ...
      const devConfig = [
        ...
        `owner: ${updateFeedOptions.owner}`,
        `repo: ${updateFeedOptions.repo}`,
        'updaterCacheDirName: com.threetwoa.app',
        '',
      ].join('\n');
   ...
   * while the downloaded archive contains the packaged app (com.threetwoa.app). Squirrel.Mac
