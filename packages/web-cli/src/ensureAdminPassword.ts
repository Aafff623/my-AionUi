...
export type EnsureAdminPasswordOptions = {
   ...
   * by launch context — packaged tarball = `threetwoa-web resetpass`, in-repo dev
...
export async function ensureAdminPassword(
  ...
  const resetCmd = opts.resetCommand ?? 'threetwoa-web resetpass';
  ...
  try {
    ...
    deps.warn(
      `[threetwoa-web] could not verify admin credentials: ${err instanceof Error ? err.message : String(err)}`
    );
    return;
  }
  ...
  if (!needsSetup) {
    ...
    deps.log(`[threetwoa-web] Log in with username "${username}". Forgot the password? Run \`${resetCmd}\`.`);
    return;
  }
  ...
  try {
    ...
    if (!resetRes.ok) {
      deps.warn(`[threetwoa-web] /api/webui/reset-password returned ${resetRes.status} — run \`${resetCmd}\``);
      return;
    }
    ...
    if (!newPassword) {
      deps.warn(`[threetwoa-web] /api/webui/reset-password returned no new_password — run \`${resetCmd}\``);
      return;
    }
    const username = await fetchAdminUsername(deps, opts.backendPort);
    deps.log(`[threetwoa-web] Generated initial admin password: ${newPassword}`);
    deps.log(`[threetwoa-web] Log in with username "${username}" and change it from the UI.`);
  } catch (err) {
    deps.warn(
      `[threetwoa-web] failed to seed initial admin password: ${err instanceof Error ? err.message : String(err)}`
    );
  }
}
