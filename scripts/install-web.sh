...
# ============================================================================
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/Aafff623/my-AionUi/main/scripts/install-web.sh | bash
#   # Or specify version:
#   VERSION=1.0.0 bash install-web.sh
#   # Or install to custom directory:
#   INSTALL_DIR=/opt/threetwoa-web bash install-web.sh
# ============================================================================
...
INSTALL_DIR="${INSTALL_DIR:-${HOME}/.local/share/threetwoa-web}"
BIN_DIR="${BIN_DIR:-${HOME}/.local/bin}"
MIRROR="${MIRROR:-https://github.com/Aafff623/my-AionUi/releases/download}"
...
Options:
  ...
  --install-dir <path>      Specify installation directory (default: ~/.local/share/threetwoa-web)
...
detect_platform_arch() {
    ...
    TARBALL_NAME="threetwoa-web-${VERSION}-${PLATFORM}-${ARCH}.tar.gz"
...
resolve_version() {
    ...
    if [[ "$VERSION" == "latest" || "$VERSION" =~ [a-zA-Z_] ]]; then
        ...
        if command -v curl &>/dev/null; then
            VERSION=$(curl -fsSL "https://api.github.com/repos/Aafff623/my-AionUi/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        elif command -v wget &>/dev/null; then
            VERSION=$(wget -qO- "https://api.github.com/repos/Aafff623/my-AionUi/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        else
...
download_tarball() {
    ...
    # Build download URL
    # MIRROR formats:
    #   - GitHub: https://github.com/Aafff623/my-AionUi/releases/download
...
extract_tarball() {
    ...
    # Extract tarball
    # Tarball root directory is threetwoa-web/, rename after extraction to INSTALL_DIR
    ...
    if [[ -d "${extract_temp}/threetwoa-web" ]]; then
        mv "${extract_temp}/threetwoa-web" "$INSTALL_DIR"
    else
        die "Tarball structure is invalid (missing threetwoa-web/ directory)"
    fi

    success "Extracted to $INSTALL_DIR"

    # Set executable permission on the bun-compiled standalone binary
    chmod +x "${INSTALL_DIR}/threetwoa-web" 2>/dev/null || true
    ...
    # Verify installation
    if [[ ! -x "${INSTALL_DIR}/threetwoa-web" ]]; then
        die "Installation failed: ${INSTALL_DIR}/threetwoa-web not found or not executable"
    fi
...
create_symlink() {
    local symlink_path="${BIN_DIR}/threetwoa-web"
    local target_path="${INSTALL_DIR}/threetwoa-web"
...
update_shell_profile() {
    ...
    # Add to profile
    echo "" >> "$profile_file"
    echo "# Added by threetwoa-web installer" >> "$profile_file"
...
print_summary() {
    ...
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo -e "  ${BOLD}📍 Symlink:${NC}                ${BIN_DIR}/threetwoa-web"
    fi
    echo ""
    ...
    if [[ "$CREATE_SYMLINK" == "1" && ":$PATH:" == *":${BIN_DIR}:"* ]]; then
        ...
        echo "    threetwoa-web start"
        echo ""
        echo "    # Check version"
        echo "    threetwoa-web version"
    else
        echo "    # Start AionUi WebUI (using full path)"
        echo "    ${INSTALL_DIR}/threetwoa-web start"
        echo ""
        echo "    # Or add symlink to PATH:"
        if [[ "$CREATE_SYMLINK" == "1" ]]; then
            echo "    export PATH=\"${BIN_DIR}:\$PATH\""
        else
            echo "    ln -s ${INSTALL_DIR}/threetwoa-web ~/.local/bin/threetwoa-web"
            echo "    export PATH=\"~/.local/bin:\$PATH\""
        fi
    fi
    echo ""
    echo -e "  ${BOLD}📖 Documentation:${NC}  https://github.com/Aafff623/my-AionUi"
    echo -e "  ${BOLD}🐛 Report issues:${NC}  https://github.com/Aafff623/my-AionUi/issues"
    echo ""
