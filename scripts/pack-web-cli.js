...
const tarballName = `threetwoa-web-${version}-${normalizedPlatform}-${normalizedArch}.tar.gz`;
...
const tarballContentDir = path.join(stagingDir, 'threetwoa-web');
...
const executableName = platform === 'win32' ? 'threetwoa-web.exe' : 'threetwoa-web';
...
// `threetwoa-web version` match the tarball filename.
...
execSync(`tar -czf ${path.basename(tarballPath)} -C ${stagingDir} threetwoa-web`, {
