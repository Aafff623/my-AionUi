...
if [ ! -d "$TEMP_DIR/threetwoa-web" ]; then
  echo "❌ Missing threetwoa-web directory"
  exit 1
fi

cd "$TEMP_DIR/threetwoa-web"

# New layout (bun compile standalone binary):
#   threetwoa-web/
#   ├── threetwoa-web           ← single compiled executable (no bin/, no dist/, no node_modules)
...
if [ ! -x "threetwoa-web" ]; then
  echo "❌ threetwoa-web is not executable"
  exit 1
fi
echo "✓ threetwoa-web is executable"

# 4. Test version command
echo ""
echo "4. Testing version command..."
VERSION=$(./threetwoa-web version)
if [ -z "$VERSION" ]; then
...
HTTP_PORT=25899
DATA_DIR="$(mktemp -d)/threetwoa-web-data"
...
./threetwoa-web start --port "$HTTP_PORT" --data-dir "$DATA_DIR" > /tmp/threetwoa-web.log 2>&1 &
SERVER_PID=$!
...
for i in $(seq 1 30); do
  if curl -sf "http://127.0.0.1:${HTTP_PORT}/" > /tmp/threetwoa-web.html 2>/dev/null; then
    break
  fi
  sleep 1
done

if [ ! -s /tmp/threetwoa-web.html ]; then
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP probe failed — no response body. Server log:"
  cat /tmp/threetwoa-web.log
  exit 1
fi

# Look for the SPA shell signature — <html + <div id="root" or similar marker
if grep -q '<html' /tmp/threetwoa-web.html && grep -qE '<(div id="root"|script)' /tmp/threetwoa-web.html; then
  echo "✓ HTTP root returns SPA index ($(wc -c < /tmp/threetwoa-web.html) bytes)"
else
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP root response does not look like SPA index:"
  head -20 /tmp/threetwoa-web.html
  echo "---server log---"
  cat /tmp/threetwoa-web.log
  exit 1
fi
...
if grep -q 'Backend binary not found' /tmp/threetwoa-web.log; then
  ...
  for i in $(seq 1 20); do
    PASSWORD=$(grep -oE 'Generated initial admin password: [^ ]+' /tmp/threetwoa-web.log | head -1 | sed 's/^Generated initial admin password: //')
    if [ -n "$PASSWORD" ]; then
      break
    fi
    sleep 1
