...
export INSTALL_DIR="/tmp/threetwoa-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/threetwoa-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/threetwoa-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/threetwoa-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/threetwoa-web"
    exit 1
fi
...
VERSION_OUTPUT=$(threetwoa-web version 2>&1 || echo "")
